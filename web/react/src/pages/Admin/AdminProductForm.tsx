...
export default function AdminProductForm() {
  ...
  const handleSyncTranslate = async () => {
    ...
    try {
      ...
      if (description.trim()) {
        fillByDetectedLang(description.trim(), {
          zh: setDescription,
          en: setDescriptionEn,
          ar: setDescriptionAr,
        })
    ...
    try {
      ...
      if (isEdit && id) {
        ...
        if (editImageItems.length > 0) {
          ...
          for (const item of editImageItems) {
            ...
            try {
              ...
              editFailures.push(item.fileName || 'image')
            }
        ...
        if (editFailures.length > 0) {
          throw new Error(
            t('admin.productForm.mediaUploadFailed').replace('{files}', editFailures.join('、')),
          )
        }
          ...
          if (item.mediaType === 'image') {
            ...
            if (fd.has('thumb') && fd.has('list') && fd.has('large') && fd.has('original')) {
              try {
                ...
                mediaFailures.push(item.fileName || 'image')
              }
            }
            setUploadState((s) => ({ ...s, uploaded: s.uploaded + 1, percent: 0 }))
            ...
            try {
              ...
              mediaFailures.push(item.fileName || 'video')
            }
            setUploadState((s) => ({ ...s, uploaded: s.uploaded + 1, percent: 0 }))
          }
        }
        ...
        if (mediaFailures.length > 0) {
          throw new Error(
            t('admin.productForm.mediaUploadFailed').replace('{files}', mediaFailures.join('、')),
          )
        }
