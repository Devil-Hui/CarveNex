...
# Idempotent: create/align the app DB user and schema from env vars.
# Runs every start on an existing datadir. Safe to call multiple times.
carvenex_ensure_user() {
	if [ -z "$MYSQL_DATABASE" ]; then
		mysql_note "CarveNex: MYSQL_DATABASE empty, nothing to ensure"
		return 0
	fi
	mysql_note "CarveNex: ensuring database '${MYSQL_DATABASE}'"
	docker_process_sql <<-EOSQL
		CREATE DATABASE IF NOT EXISTS \`${MYSQL_DATABASE}\` ;
	EOSQL
	mysql_note "CarveNex: ensuring user '${MYSQL_USER}'@'%'"
	docker_process_sql <<-EOSQL
...
_main() {
	...
	if [ "$1" = 'mysqld' ] && ! _mysql_want_help "$@"; then
		...
		if [ -z "$DATABASE_ALREADY_EXISTS" ]; then
			...
			mysql_socket_fix
			# CarveNex hardening: existing data directory -> still make sure the
			# app DB user/password from env match, so changing credentials in
			# .env doesn't cause "Access denied" (Docker MySQL only runs
			# docker_setup_db on an empty datadir). Reuse the temp-server path.
			if [ -n "$MYSQL_USER" ] && [ -n "$MYSQL_PASSWORD" ]; then
				mysql_note "CarveNex: aligning DB user '${MYSQL_USER}' (existing datadir)"
				docker_temp_server_start "$@"
				mysql_socket_fix
				carvenex_ensure_user
				docker_temp_server_stop
				mysql_note "CarveNex: DB user aligned"
			fi
		fi
	fi
	exec "$@"
