...
在将 API Schema 交给前端 / 测试 / 外部文档使用前，执行导出前清洗：
  ...
  3. 输出 YAML（docs/api/carvenex-openapi-safe.yml）并打印清洗统计

用法（docker 内，与契约基线同源）：
  python scripts/sanitize_openapi.py [--output docs/api/carvenex-openapi-safe.yml]
"""
...
_DOMAIN_PLACEHOLDERS = [
    (re.compile(r'https?://(?:api|admin|shop|www)\.carvenex\.com', re.I), '${API_BASE_URL}'),
...
def _sanitize_value(value, key: str = '') -> str:
    ...
    if lowered in _SENSITIVE_KEYS and len(value) < 64:
        return '<REDACTED>'
    if lowered in ('url', 'server', 'base_url', 'host') and 'carvenex' in value.lower():
        for pattern, repl in _DOMAIN_PLACEHOLDERS:
...
def main() -> int:
    ...
    parser.add_argument(
        '--output',
        default=str(BACKEND_ROOT.parent / 'docs/api/carvenex-openapi-safe.yml'),
        help='输出路径（默认仓库根 docs/api/carvenex-openapi-safe.yml）',
    )
