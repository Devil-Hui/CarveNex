...
def _apifox_environment(name, base_url):
    return {
        'id': f'carvenex-{name.lower().replace(" ", "-")}',
        'name': name,
        'values': [
        ...
        '_postman_exported_using': 'CarveNex generate_api_docs',
    }
...
def _render_reference(schema):
    lines = [
        ...
        '3. 浏览器自动携带 `carvenex_access` / `carvenex_refresh` HttpOnly Cookie；写请求继续携带 `X-CSRFToken`。',
        '',
        '### Bearer 客户端',
        '',
